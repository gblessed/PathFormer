...
# Valid keys for known_for_channel: use GT for these when building the predicted channel.
KNOWN_CHANNEL_KEYS = {"delay", "power", "phase", "aoa_az", "aoa_el"}


def _channel_inputs_for_pred(gt, pred, known_for_channel=None):
    """
    Build channel inputs for the *predicted* channel, optionally using GT for some dimensions.
    gt, pred: (T, 5) with [delay, power, phase, aoa_az, aoa_el].
    known_for_channel: None or sequence of keys in KNOWN_CHANNEL_KEYS. When provided, use GT
        for those dimensions and pred for the rest. When None or empty, use all pred.
    Returns: (delay_secs, power_linear, phase_degs, az, el) for generate_MIMO_channel_torch.
    """
    known = set(known_for_channel or [])
    known = known & KNOWN_CHANNEL_KEYS
    delay_secs = (gt[:, 0] / 1e6) if "delay" in known else (pred[:, 0] / 1e6)
    ...
    power_linear = power_linear_gt if "power" in known else power_linear_pred
    phase_degs_gt = torch.rad2deg(gt[:, 2])
    phase_degs_pred = torch.rad2deg(pred[:, 2])
    phase_degs = phase_degs_gt if "phase" in known else phase_degs_pred
    az = gt[:, 3] if "aoa_az" in known else pred[:, 3]
    el = gt[:, 4] if "aoa_el" in known else pred[:, 4]
    return delay_secs, power_linear, phase_degs, az, el
...
def evaluate_model(
    model,
    val_loader,
    train_data_pad_value=0,
    max_generate=26,
    log_to_wandb=False,
    known_for_channel=None,
):
    """
    known_for_channel: None or list/set of keys in {'delay','power','phase','aoa_az','aoa_el'}.
      When provided, the predicted channel is built using GT for those dimensions and
      predictions for the rest. When None or empty, channel uses all generated path metrics.
    """
    model.eval()
            ...
            for b in inner_bar:
                ...
                if T > 0:
                    ...
                    (
                        ...
                        az_pred,
                        el_pred,
                    ) = _channel_inputs_for_pred(gt, pred, known_for_channel)
                    array_resp_pred = compute_single_array_response_torch(
                        params.bs_antenna, az_pred.unsqueeze(0), el_pred.unsqueeze(0)
                    )
    ...
    known_str = ", ".join(sorted(known_for_channel)) if known_for_channel else "none (all predicted)"
    print("\n=================  Final EVALUATION RESULTS =================")
    print(f"Channel built with known (GT): [{known_str}]")
...
def evaluate_model_batch(
    ...
    known_for_channel: None or list/set of {'delay','power','phase','aoa_az','aoa_el'} to use GT for those when building pred channel.
    """
    model.eval()
    CH_SCORE_DB_MIN, CH_SCORE_DB_MAX = -20.0, 0.0
    delay_errors = []
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        metavar="KEY",
        help="Use GT for these when building predicted channel: delay, power, phase, aoa_az, aoa_el. Default: none (all predicted). E.g. --known_for_channel delay power",
    )
    ...
    known_for_channel = args.known_for_channel
    if known_for_channel is not None:
        known_for_channel = [k.strip().lower() for k in known_for_channel if k.strip()]
        known_for_channel = [k for k in known_for_channel if k in KNOWN_CHANNEL_KEYS]
        if not known_for_channel:
            known_for_channel = None
    ...
    results = evaluate_model_batch(
        ...
        known_for_channel=known_for_channel,
    )
    (
        avg_delay,
        avg_power,
    ...
    known_str_csv = ",".join(sorted(known_for_channel)) if known_for_channel else "none"
    row = {
        "scenario": scenario,
        "path_checkpoint": path_checkpoint_path,
        "known_for_channel": known_str_csv,
