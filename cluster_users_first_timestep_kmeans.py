...
DEFAULT_OUTPUT = Path(__file__).resolve().parent / "first_timestep_user_kmeans.pdf"
...
def collect_first_timestep_features(datasets):
    ...
    tx_groups = sorted(df["tx_group"].unique())
    tx_to_idx = {tx_group: tx_idx for tx_idx, tx_group in enumerate(tx_groups)}
    df["tx_idx"] = df["tx_group"].map(tx_to_idx).astype(int)
    return df
...
def cluster_users(df, args):
    ...
    for tx_idx, tx_df in df.groupby("tx_idx", sort=True):
        tx_df = tx_df.copy()
        features = tx_df[feature_cols].to_numpy(dtype=np.float32)
        ...
        if len(tx_df) < args.n_clusters:
            raise ValueError(
                f"Requested n_clusters={args.n_clusters}, but TX {tx_idx} only has {len(tx_df)} users with a first valid path."
            )
        ...
        tx_df["cluster"] = labels.astype(int)
        clustered_parts.append(tx_df)
        ...
        centers_df.insert(0, "tx_idx", int(tx_idx))
        tx_group = tx_df["tx_group"].iloc[0]
        centers_df.insert(1, "tx_group", tx_group)
        center_parts.append(centers_df)

    return pd.concat(clustered_parts, ignore_index=True), pd.concat(center_parts, ignore_index=True)


def build_cluster_summary(df, centers_df):
    summary = (
        df.groupby(["tx_idx", "tx_group", "cluster"], as_index=False)
        .agg(
    ...
    summary = summary.merge(
        ...
        on=["tx_idx", "tx_group", "cluster"],
        how="left",
    )
    return summary.sort_values(["tx_idx", "cluster"]).reset_index(drop=True)


def plot_clusters(df, centers_df, args, tx_idx, output_path):
    clusters = np.sort(df["cluster"].unique())
    cmap = plt.get_cmap("tab10", len(clusters))
    tx_x = float(df["tx_x"].iloc[0])
    tx_y = float(df["tx_y"].iloc[0])
    ...
    for cluster in clusters:
        ...
        ax_users.scatter(
            ...
            alpha=0.75,
            color=color,
            # label=f"cluster {cluster} (n={len(cluster_df)})",s
        )
        ax_features.scatter(
            cluster_df["delay_us"],
    ...
    ax_users.set_xlabel("x")
    ax_users.set_ylabel("y")
    ax_users.scatter(
        [tx_x],
        [tx_y],
        s=220,
    ...
    ax_users.legend(loc="best", fontsize=16)
    ...
    ax_features.legend(loc="best", fontsize=12)

    # fig.suptitle(
    #     f"KMeans clustering from first path delay/power\n"
    #     f"scenario={args.scenario}, tx_idx={tx_idx}, split={args.split}, sort_by={args.sort_by}, n_clusters={args.n_clusters}"
    # )
...
def output_path_for_tx(base_output, tx_idx, multiple_txs):
    if not multiple_txs:
        return base_output
    return base_output.with_name(f"{base_output.stem}_tx{tx_idx}{base_output.suffix}")
...
def main():
    ...
    multiple_txs = clustered_df["tx_idx"].nunique() > 1
    saved_figures = []
    for tx_idx, tx_df in clustered_df.groupby("tx_idx", sort=True):
        tx_centers_df = centers_df[centers_df["tx_idx"] == tx_idx].copy()
        output_path = output_path_for_tx(args.output, int(tx_idx), multiple_txs)
        plot_clusters(tx_df.copy(), tx_centers_df, args, int(tx_idx), output_path)
