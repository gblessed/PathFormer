...
from multiscenario_direct_training import load_best_checkpoint as load_direct_checkpoint
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Beam head finetuning for direct, first-step residual, and corridor model families."
    )
    ...
    parser.add_argument("--beam-head-checkpoint-dir", type=str, default="/home/blessedg/Pathformer/checkpoints_beam_heads")
...
def build_residual_loaders(dataset, batch_size, pad_value, n_clusters, num_workers):
    ...
    train_aug_prompts, train_baselines, val_aug_prompts, val_baselines = build_first_step_assignments(
        base_train, base_val, n_clusters=n_clusters
    )
...
class BeamHeadFinetuner(nn.Module):
    def __init__(self, backbone, model_family, hidden_dim=512, n_beams=64):
        super().__init__()
...
def checkpoint_specs_for_scenario(args, scenario):
    return [
        {
            "model_family": "direct",
            "backbone_checkpoint_path": os.path.join(args.checkpoint_root_direct, f"multiscenario_direct_{scenario}_best_model_checkpoint.pth"),
        },
        {
            "model_family": "first_step_residual",
            "backbone_checkpoint_path": os.path.join(args.checkpoint_root_residual, f"first_step_residual_{scenario}_best_model_checkpoint.pth"),
        },
        {
            "model_family": "first_step_residual_corridor",
            "backbone_checkpoint_path": os.path.join(args.checkpoint_root_corridor, f"first_step_residual_corridor_{scenario}_best_model_checkpoint.pth"),
        },
    ]
...
def main():
    ...
    print(f"Running beam finetuning for {len(scenarios)} scenario(s): {scenarios}")

    for scenario in scenarios:
        print(f"\nBeam finetuning for {scenario}")
        dataset = dm.load(scenario)
        for spec in checkpoint_specs_for_scenario(args, scenario):
            model_family = spec["model_family"]
            backbone_checkpoint_path = spec["backbone_checkpoint_path"]
            if not os.path.exists(backbone_checkpoint_path):
                print(f"Skipping {scenario} [{model_family}] because backbone checkpoint is missing: {backbone_checkpoint_path}")
                continue
            ...
            model, train_loader, val_loader = build_family_objects(dataset, model_family, args, config)
            print(f"{model_family} trainable head parameters: {count_parameters(model.beam_head)}")
            head_checkpoint_path = os.path.join(
                args.beam_head_checkpoint_dir,
                f"beam_head_{model_family}_{scenario}.pth",
            )
            ...
            print(
                f"{scenario} [{model_family}] | top1={metrics['top1_acc']:.4f}, "
