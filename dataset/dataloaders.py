...
class MySeqDataLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, scenario, tx_sets="all", seed=42, shuffle=False, pad_value=0, 
                 train=True, split_by="users", train_ratio=0.8, sort_by="power", include_aod=False):
    ...
    def __getitem__(self, idx):
        ...
        paths.append([0.0] * len(path_keys))
        ...
        for step_idx, indx in enumerate(indices):
            ...
            for k in path_keys:
                ...
                elif k in ["aoa_az", "aoa_el", "aod_az", "aod_el"]:
...
class PreTrainMySeqDataLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, scenario, tx_sets="all", seed=42, shuffle=False, pad_value=0,
                 train=True, split_by="users", train_ratio=0.8, sort_by="power", normalizers = None, apply_normalizers= [], include_aod=False):
        ### get length of dataset
        self.dataset = scenario
        self.Txs = 1
        self.pad_value = pad_value
...
class PreTrainMySeqDataLoaderAblate(torch.utils.data.Dataset):

    def __init__(self, scenario, tx_sets="all", seed=42, shuffle=False, pad_value=0,
                 train=True, split_by="users", train_ratio=0.8, acutal_ratio=0.8, sort_by="power", normalizers = None, apply_normalizers= [], include_aod=False):
        ### get length of dataset
        self.dataset = scenario
        self.Txs = 1
        self.pad_value = pad_value
