...
DEFAULT_CSV_LOG = "/home/blessedg/Pathformer/logs/channel_finetune_embedding_all_families_foundation.csv"
...
def parse_args():
    ...
    parser.add_argument("--embed-pool", choices=["first", "last", "mean", "mean_last", "mean_first_last"], default="mean")
    parser.add_argument("--feature-source", choices=["rollout", "teacher_forced"], default="rollout")
    parser.add_argument("--head-hidden-mult", type=int, default=2)
    parser.add_argument("--head-depth", type=int, default=1)
    parser.add_argument("--head-dropout", type=float, default=0.1)
    parser.add_argument("--loss-mode", choices=["mse", "phase_aligned", "magnitude"], default="mse")
    ...
    parser.add_argument("--normalization", choices=["rms", "mean"], default="mean")
...
def build_channel_targets(dataset, channels, train, seed=42, train_ratio=0.8):
    ...
    for tx in range(len(dataset_list)):
        ...
        channel_tx = normalize_channel_tensor(np.array(channels[:, tx]))
        n_ue = data_tx.n_ue
        ...
        indices = indices[:split_idx] if train else indices[split_idx:]
...
class ChannelHeadFromPathEmbeddings(nn.Module):
    def __init__(
        ...
        hidden_dim: int,
        out_numel: int,
        pool_mode: Literal["first", "last", "mean", "mean_last", "mean_first_last"] = "mean",
        max_generated_steps: int = 25,
        feature_source: Literal["rollout", "teacher_forced"] = "rollout",
        ...
        pooled_dim = self._pooled_dim(hidden_dim, pool_mode)
        head_width = max(pooled_dim, hidden_dim * head_hidden_mult)
        layers = []
        in_dim = pooled_dim
        for _ in range(head_depth):
            ...
            in_dim = head_width
        layers.append(nn.Linear(in_dim, out_numel))
        self.channel_head = nn.Sequential(*layers)

    @staticmethod
    def _pooled_dim(hidden_dim, pool_mode):
        if pool_mode == "mean_last":
            return hidden_dim * 2
        if pool_mode == "mean_first_last":
            return hidden_dim * 3
        return hidden_dim
    ...
    def _pool(self, h_paths):
        ...
        if self.pool_mode == "last":
            return h_paths[:, -1, :]
        if self.pool_mode == "mean_last":
            return torch.cat([h_paths.mean(dim=1), h_paths[:, -1, :]], dim=-1)
        if self.pool_mode == "mean_first_last":
            return torch.cat([h_paths.mean(dim=1), h_paths[:, 0, :], h_paths[:, -1, :]], dim=-1)
        return h_paths.mean(dim=1)

    def extract_pooled_features(self, prompts, paths_in=None, interactions_in=None, first_step_baseline=None):
        if self.feature_source == "teacher_forced":
            if paths_in is None or interactions_in is None:
                raise ValueError("teacher_forced features require paths_in and interactions_in.")
            h_paths = self._forward_hidden(prompts, paths_in, interactions_in)
            return self._pool(h_paths)

        if self.pool_mode == "first":
            h_paths = self._rollout_hidden(prompts, 1, first_step_baseline)
            return h_paths[:, 0, :]
    ...
    def predict_from_features(self, pooled):
        ...
        m_tx = int(np.prod(params.bs_antenna[c.PARAMSET_ANT_SHAPE]))
        m_rx = int(np.prod(params.ue_antenna[c.PARAMSET_ANT_SHAPE]))
        real = real.view(pooled.size(0), m_rx, m_tx, n_sc)
        imag = imag.view(pooled.size(0), m_rx, m_tx, n_sc)
    ...
    def forward(self, prompts, paths_in=None, interactions_in=None, first_step_baseline=None):
        pooled = self.extract_pooled_features(prompts, paths_in, interactions_in, first_step_baseline)
        return self.predict_from_features(pooled)


class CachedChannelFeatureDataset(torch.utils.data.Dataset):
    def __init__(self, features, targets, target_scale, target_mean, target_std):
        self.features = features.float().cpu()
        self.targets = targets.to(torch.complex64).cpu()
...
def compute_global_channel_rms(targets):
    power = (targets.real ** 2 + targets.imag ** 2).mean()
    return torch.sqrt(power).clamp(min=1e-8)

def compute_global_channel_mean_std(targets):
    return targets.mean(), targets.std()


def slice_cached_dataset(dataset, n_samples):
    if n_samples <= 0:
        return dataset
    n = min(n_samples, len(dataset))
    return CachedChannelFeatureDataset(
        dataset.features[:n],
        dataset.targets[:n],
...
def make_foundation_path_model(prompt_dim, checkpoint_path):
...
#     # target_scale = compute_global_channel_rms(gt_ch)
#     # gt_norm = gt_ch / target_scale
#     # pred_norm = pred_ch / target_scale
#     mse = ((gt_ch.real - pred_ch.real) ** 2 + (gt_ch.imag - pred_ch.imag) ** 2).mean()
#     gt_ch_sq = (gt_ch.real ** 2 + gt_ch.imag ** 2).mean().clamp(min=1e-8)

#     return mse / (gt_ch_sq + 1e-10)

def complex_channel_loss(pred_ch, gt_ch, loss_mode="mse"):
    if loss_mode == "phase_aligned":
        reduce_dims = tuple(range(1, pred_ch.dim()))
        denom = (pred_ch.real ** 2 + pred_ch.imag ** 2).sum(dim=reduce_dims, keepdim=True).clamp(min=1e-8)
        alpha = (gt_ch * pred_ch.conj()).sum(dim=reduce_dims, keepdim=True) / denom
        pred_ch = alpha * pred_ch
        mse = ((gt_ch.real - pred_ch.real) ** 2 + (gt_ch.imag - pred_ch.imag) ** 2).mean()
    elif loss_mode == "magnitude":
        mse = ((gt_ch.abs() - pred_ch.abs()) ** 2).mean()
    else:
        mse = ((gt_ch.real - pred_ch.real) ** 2 + (gt_ch.imag - pred_ch.imag) ** 2).mean()

    gt_norm_sq = (gt_ch.real ** 2 + gt_ch.imag ** 2).mean().clamp(min=1e-8)
    return mse, mse / gt_norm_sq


def maybe_denormalize_cached_channels(dataset, pred_ch, gt_ch):
    if getattr(dataset, "target_mean", None) is not None and getattr(dataset, "target_std", None) is not None:
        mean = torch.as_tensor(dataset.target_mean, device=pred_ch.device, dtype=pred_ch.dtype)
        std = torch.as_tensor(dataset.target_std, device=pred_ch.device, dtype=pred_ch.dtype)
        return pred_ch * std + mean, gt_ch * std + mean
    return pred_ch, gt_ch


def channel_loss_for_batch(model, batch, model_family, loss_mode="mse", scale=1e6):
    features, gt_ch = batch
    features = features.to(device)
    gt_ch = gt_ch.to(device)
    pred_ch = model.predict_from_features(features)
    return complex_channel_loss(pred_ch, gt_ch, loss_mode)

def channel_loss_from_cached_batch(model, batch, target_scale, loss_mode="mse", scale=1e6):
    features, gt_ch = batch
    features = features.to(device)
    gt_ch = gt_ch.to(device)
    pred_ch = model.predict_from_features(features)
    target_scale = torch.as_tensor(target_scale, device=gt_ch.device, dtype=gt_ch.real.dtype)
    gt_norm = gt_ch / target_scale
    pred_norm = pred_ch / target_scale
    loss, _ = complex_channel_loss(pred_norm, gt_norm, loss_mode)
    return loss


def precompute_cached_channel_features(model, loader, model_family, desc, normalization):
    model.eval()
    ...
    with torch.no_grad():
        for batch in tqdm(loader, desc=desc, leave=False):
            ...
            paths_in = paths[:, :-1, :]
            interactions_in = interactions[:, :-1, :]
            pooled = model.extract_pooled_features(prompts, paths_in, interactions_in, first_step_baseline)
            feature_batches.append(pooled.detach().cpu())
            target_batches.append(gt_ch.detach().cpu())
    features = torch.cat(feature_batches, dim=0)
    targets = torch.cat(target_batches, dim=0)
    target_scale = None
    target_mean, target_std = None, None
    if normalization == "rms":
        target_scale = compute_global_channel_rms(targets)
    else:
        target_mean, target_std = compute_global_channel_mean_std(targets)
    return CachedChannelFeatureDataset(features, targets, target_scale, target_mean, target_std)


def evaluate_channel_head(model, val_loader, model_family):
    model.eval()
    scale = 1e6
    nmses_raw, nmse_logs, nmse_dbs, scores = [], [], [], []
    use_cached_features = hasattr(val_loader.dataset, "target_scale")
    with torch.no_grad():
        for batch in tqdm(val_loader, desc=f"Eval channel head [{model_family}]", leave=False):
            if use_cached_features:
                ...
                gt_ch = gt_ch.to(device)
                pred_ch = model.predict_from_features(features)
                pred_ch, gt_ch = maybe_denormalize_cached_channels(val_loader.dataset, pred_ch, gt_ch)
            else:
                ##bad! should be autoregreesive
                prompts, paths, interactions, path_padding_mask, first_step_baseline, gt_ch = unpack_batch(batch, model_family)
                paths_in = paths[:, :-1, :]
                interactions_in = interactions[:, :-1, :]
                pred_ch = model(prompts, paths_in, interactions_in, first_step_baseline)
            # Compare only the last 16 subcarriers on the final tensor axis.
            # pred_ch = pred_ch[..., 16:]
            # gt_ch = gt_ch[..., 16:]

            gt_s = gt_ch * scale
            pred_s = pred_ch * scale
            reduce_dims = tuple(range(1, gt_s.dim()))
            mse = ((gt_s.real - pred_s.real) ** 2 + (gt_s.imag - pred_s.imag) ** 2).mean(dim=reduce_dims)
            gt_norm_sq = (gt_s.real ** 2 + gt_s.imag ** 2).mean(dim=reduce_dims).clamp(min=1e-6)
            nmse = (mse / (gt_norm_sq + 1e-10)).detach().cpu().numpy()
            nmses_raw.extend(nmse.tolist())
    ...
    return {
        "nmse_raw": float(np.mean(nmses_raw)) if nmses_raw else 0.0,
...
def train_channel_head(model, train_loader, val_loader, model_family, args, checkpoint_path, scenario):
    ...
    target_scale = getattr(train_loader.dataset, "target_scale", None)
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"{scenario} [{model_family}] epoch {epoch} train", leave=False):
            if target_scale is None:
                loss, nmse = channel_loss_for_batch(model, batch, model_family, args.loss_mode)
                train_nmses.append(nmse.item())
            else:
                loss = channel_loss_from_cached_batch(model, batch, target_scale, args.loss_mode)
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"{scenario} [{model_family}] epoch {epoch} val", leave=False):
                if target_scale is None:
                    ...
                    val_nmses.append(nmse.item())
                else:
                    val_losses.append(channel_loss_from_cached_batch(model, batch, target_scale, args.loss_mode).item())
        val_loss = float(np.mean(val_losses)) if val_losses else float("inf")
        val_nmse= float(np.mean(val_nmses)) if train_losses else float("inf")
        train_loss = float(np.mean(train_losses)) if train_losses else float("inf")
        train_nmse = float(np.mean(train_nmses)) if train_losses else float("inf")

        print(
            f"{scenario} [{model_family}] epoch {epoch:03d} "
            f"train_ch_loss={train_loss:.4f} train_nmse={train_nmse} val_ch_loss={val_loss:.4f} val_nmse={val_nmse:.4f}"
        )
...
def run_scenario_family(dataset, channel_targets, scenario, model_family, path_checkpoint, args):
    ...
    pad_value = 0
    train_data, val_data, _, _, prompt_dim = build_corridor_loaders(dataset, args, pad_value)
    train_loader, val_loader = attach_channel_targets(train_data, val_data, args, channel_targets)

    path_model = make_foundation_path_model(prompt_dim, path_checkpoint)
    ...
    if args.use_cached_features:
        cached_train = precompute_cached_channel_features(
            model,
            train_loader,
            "first_step_residual_corridor",
            f"Cache train features [{scenario}]",
            args.normalization,
        )
        print("[] caching val data")
        cached_val = precompute_cached_channel_features(
            model,
            val_loader,
            "first_step_residual_corridor",
            f"Cache val features [{scenario}]",
            args.normalization,
        )
        cached_val.target_scale = cached_train.target_scale
        cached_val.target_mean = cached_train.target_mean
        cached_val.target_std = cached_train.target_std
        if args.normalization == "mean":
            cached_train.targets = (cached_train.targets - cached_train.target_mean)/ cached_train.target_std
            cached_val.targets = (cached_val.targets - cached_train.target_mean)/ cached_train.target_std

        if args.overfit_samples > 0:
            cached_train = slice_cached_dataset(cached_train, args.overfit_samples)
            cached_val = slice_cached_dataset(cached_val, min(args.overfit_samples, len(cached_val)))
            print(f"Using overfit subset: train={len(cached_train)} val={len(cached_val)}")


        train_loader = DataLoader(cached_train, batch_size=args.batch_size, shuffle=True)
        val_loader = DataLoader(cached_val, batch_size=args.batch_size, shuffle=False)
...
def run_scenario(scenario, args):
    ...
    print(f"channels shape all {channel_targets.shape}")
    rows = []
    model_family = "foundation_corridor_concat"
    row = run_scenario_family(dataset, channel_targets, scenario, model_family, args.pretrained_checkpoint, args)
    if row is not None:
        rows.append(row)
