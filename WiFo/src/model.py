...
class WiFo(nn.Module):
    ...
    def __init__(self, patch_size=1, in_chans=1,
        ...
        self.input_noise_snr_db = float(getattr(args, "input_noise_snr_db", 20.0))
        self.freeze_wifo_backbone = bool(getattr(args, "freeze_wifo_backbone", False))
        self.train_decoder_pred = bool(getattr(args, "train_decoder_pred", False))
        self.adapter_mode = str(getattr(args, "adapter_mode", "none"))
        self.pathformer_feature_dim = int(getattr(args, "pathformer_feature_dim", 0))
        self.pathformer_token_steps = int(getattr(args, "pathformer_token_steps", 25))
        self.context_fusion_position = str(getattr(args, "context_fusion_position", "after_decoder"))
        if self.context_fusion_position not in {"after_decoder", "before_decoder"}:
            raise ValueError(
                "context_fusion_position must be one of: after_decoder, before_decoder"
            )
        self.context_feature_proj = None
        self.decoder_context_fusion_mlp = None
        self.learned_context_features = None
        self.context_token_proj = None
        self.decoder_context_cross_attn = None
        self.decoder_context_cross_attn_norm = None
        self.decoder_context_gate = None
        self.learned_context_tokens = None
        if self.adapter_mode in {"learned", "pathformer"}:
            self.context_feature_proj = nn.Linear(self.pathformer_feature_dim, decoder_embed_dim)
            self.decoder_context_fusion_mlp = nn.Sequential(
                nn.Linear(decoder_embed_dim * 2, decoder_embed_dim),
                nn.GELU(),
                nn.Linear(decoder_embed_dim, decoder_embed_dim),
            )
            if self.adapter_mode == "learned":
                self.learned_context_features = nn.Parameter(
                    torch.zeros(1, self.pathformer_feature_dim)
                )
                nn.init.normal_(self.learned_context_features, std=0.02)
            self._configure_finetune_trainables()
        elif self.adapter_mode in {"learned_tokens", "pathformer_tokens"}:
            self.context_token_proj = nn.Linear(self.pathformer_feature_dim, decoder_embed_dim)
            self.decoder_context_cross_attn = nn.MultiheadAttention(
                embed_dim=decoder_embed_dim,
                num_heads=decoder_num_heads,
                batch_first=True,
            )
            self.decoder_context_cross_attn_norm = norm_layer(decoder_embed_dim)
            self.decoder_context_gate = nn.Parameter(
                torch.tensor(float(getattr(args, "context_gate_init", 0.0)))
            )
            if self.adapter_mode == "learned_tokens":
                self.learned_context_tokens = nn.Parameter(
                    torch.zeros(1, self.pathformer_token_steps, self.pathformer_feature_dim)
                )
                nn.init.normal_(self.learned_context_tokens, std=0.02)
            self._configure_finetune_trainables()
    ...
    def _configure_finetune_trainables(self):
        if not self.freeze_wifo_backbone:
            return
        ...
        for module in (
            self.context_feature_proj,
            self.decoder_context_fusion_mlp,
            self.context_token_proj,
            self.decoder_context_cross_attn,
            self.decoder_context_cross_attn_norm,
        ):
            if module is None:
                continue
        ...
        if self.learned_context_features is not None:
            self.learned_context_features.requires_grad = True
        if self.learned_context_tokens is not None:
            self.learned_context_tokens.requires_grad = True
        if self.decoder_context_gate is not None:
            self.decoder_context_gate.requires_grad = True

        if self.train_decoder_pred:
            for param in self.decoder_pred.parameters():
                param.requires_grad = True

    def _resolve_context_features(self, batch_size, context_features):
        if self.adapter_mode == "none":
            return None
        if self.adapter_mode == "learned":
            if self.learned_context_features is None:
                return None
            return self.learned_context_features.expand(batch_size, -1)
        if self.adapter_mode == "learned_tokens":
            if self.learned_context_tokens is None:
                return None
            return self.learned_context_tokens.expand(batch_size, -1, -1)
        if self.adapter_mode == "pathformer":
            return context_features
        if self.adapter_mode == "pathformer_tokens":
            return context_features
        raise ValueError(f"Unknown adapter_mode: {self.adapter_mode}")

    def _fuse_context_features(self, decoder_tokens, context_features):
        resolved_context = self._resolve_context_features(decoder_tokens.shape[0], context_features)
        if resolved_context is None:
            return decoder_tokens

        if resolved_context.dim() == 2:
            if self.context_feature_proj is None or self.decoder_context_fusion_mlp is None:
                return decoder_tokens
            projected_context = self.context_feature_proj(resolved_context).unsqueeze(1)
            projected_context = projected_context.expand(-1, decoder_tokens.shape[1], -1)
            fused_tokens = torch.cat([decoder_tokens, projected_context], dim=-1)
            return decoder_tokens + self.decoder_context_fusion_mlp(fused_tokens)

        if resolved_context.dim() == 3:
            if (
                self.context_token_proj is None
                or self.decoder_context_cross_attn is None
                or self.decoder_context_cross_attn_norm is None
            ):
                return decoder_tokens
            projected_tokens = self.context_token_proj(resolved_context)
            query_tokens = self.decoder_context_cross_attn_norm(decoder_tokens)
            attn_out, _ = self.decoder_context_cross_attn(
                query=query_tokens,
                key=projected_tokens,
                value=projected_tokens,
                need_weights=False,
            )
            gate = torch.sigmoid(self.decoder_context_gate) if self.decoder_context_gate is not None else 1.0
            return decoder_tokens + gate * attn_out
    ...
    def forward_decoder(self, x, ids_restore, mask_strategy, input_size=None, data=None, scale=None, context_features=None):
        N = x.shape[0]
        T, H, W = input_size
        ...
        if self.context_fusion_position == "before_decoder":
            x_attn = self._fuse_context_features(x_attn, context_features)

        # apply Transformer blocks
        for index, blk in enumerate(self.decoder_blocks):
            x_attn = blk(x_attn)
        x_attn = self.decoder_norm(x_attn)
        if self.context_fusion_position == "after_decoder":
            x_attn = self._fuse_context_features(x_attn, context_features)
    ...
    def forward(self, imgs, mask_ratio=0.5, mask_strategy='random',seed=None, data='none', pathformer_features=None):

        imgs = torch.stack(imgs).squeeze(1)
        # print(f"imgs: {imgs.shape}")
        imgs_n = imgs
        if self.input_noise_snr_db >= 0:
            noise_power = torch.mean(imgs ** 2) * 10 ** (-self.input_noise_snr_db / 10)
        ...
        pred = self.forward_decoder(
            ...
            data=data,
            scale=scale,
            context_features=pathformer_features,
        )  # [N, L, p*p*1]
