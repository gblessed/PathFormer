#!/bin/bash
#SBATCH --job-name=mm-los-foundation
...
SCRIPT_PATH="${PROJECT_DIR}/pre_train_finetune_los_prediction_foundation.py"
...
LOS_HEAD_CHECKPOINT_DIR="${LOS_HEAD_CHECKPOINT_DIR:-${PROJECT_DIR}/checkpoints_los_heads_foundation}"
RESULT_PREFIX="${RESULT_PREFIX:-los_foundation}"
...
TRAIN_VAL_RATIO="${TRAIN_VAL_RATIO:-0.125}"
MAX_GENERATE="${MAX_GENERATE:-25}"
USE_MATERIAL_FEATURES="${USE_MATERIAL_FEATURES:-1}"
MAX_JOBS="${MAX_JOBS:-12}"
SKIP_TRAIN="${SKIP_TRAIN:-0}"

mkdir -p "${LOG_DIR}"
mkdir -p "${LOS_HEAD_CHECKPOINT_DIR}"
...
echo "LOS head checkpoints: ${LOS_HEAD_CHECKPOINT_DIR}"
...
for scenario in "${SCENARIOS[@]}"; do
    ...
    python "${SCRIPT_PATH}" \
        ...
        --los-head-checkpoint-dir "${LOS_HEAD_CHECKPOINT_DIR}" \
...
echo "Finished los-foundation runs for ${#SCENARIOS[@]} scenarios."
