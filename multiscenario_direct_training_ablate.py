...
def evaluate_model(model, val_loader, max_generate=26, log_to_wandb=False):
    ...
    avg_aod_az = np.mean(aod_az_errors) if len(aod_az_errors) > 0 else 0.0
    avg_aod_el = np.mean(aod_el_errors) if len(aod_el_errors) > 0 else 0.0
    ...
    avg_aod_az_mae = np.mean(aod_az_maes)
    avg_aod_el_mae = np.mean(aod_el_maes)
    ...
    print(f"AoD Azimuth RMSE     : {avg_aod_az:.4f} degrees")
    print(f"AoD Elevation RMSE   : {avg_aod_el:.4f} degrees")
    ...
    print(f"AoD Azimuth MAE     : {avg_aod_az_mae:.4f} degrees")
    print(f"AoD Elevation MAE   : {avg_aod_el_mae:.4f} degrees")
...
def run_scenario(scenario, args):
    ...
    config = {
        ...
        "experiment": f"multiscenario_direct_{scenario}_{args.train_ratio}_seed{args.seed}",
        "hidden_dim": 512,
        "n_layers": 8,
        "n_heads": 8,
    ...
    train_data = PreTrainMySeqDataLoaderAblate(dataset, train=True, split_by="user", sort_by="power", normalizers=None, apply_normalizers=[], train_ratio=args.train_ratio, seed=args.seed, pad_value=config["PAD_VALUE"], include_aod=True)
    ...
    val_data = PreTrainMySeqDataLoaderAblate(dataset, train=False, split_by="user", sort_by="power", normalizers=None, apply_normalizers=[], train_ratio=args.train_ratio, seed=args.seed, pad_value=config["PAD_VALUE"], include_aod=True)
    ...
    print(f"Num_train: {len(train_data)}")
    print(f"Num_test: {len(val_data)}")
    ...
    if len(results) == 18:
        ...
        std_delay = std_power = std_phase = std_az = std_el = np.nan
        std_aod_az = std_aod_el = std_path_length_rmse = np.nan
        std_interaction_accuracy = std_interaction_f1 = np.nan
        std_delay_mae = std_power_mae = std_phase_mae = np.nan
        std_az_mae = std_el_mae = std_aod_az_mae = std_aod_el_mae = np.nan
        std_path_length_mae = np.nan
    else:
        (
            avg_delay,
            std_delay,
