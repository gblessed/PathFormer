...
def build_first_step_assignments(train_data, val_data, n_clusters, seed=42):
...
def evaluate_model(model, val_loader, max_generate=25):
    ...
    return (
        ...
        np.mean(az_errors), np.mean(el_errors), np.mean(aod_az_errors), np.mean(aod_el_errors), np.mean(path_length_rmses),
        avg_interaction_accuracy, avg_interaction_f1,
        np.mean(delay_maes), np.mean(power_maes), np.mean(phase_maes),
        np.mean(az_maes), np.mean(el_maes), np.mean(aod_az_maes), np.mean(aod_el_maes), np.mean(path_length_maes),
    )
...
def run_scenario(scenario, args):
    ...
    config = {
        ...
        "experiment": f"first_step_residual_{scenario}_{args.train_ratio}_seed{args.seed}",
        "hidden_dim": 512,
        "n_layers": 8,
        "n_heads": 8,
    ...
    print(f"Num_train: {len(base_train)}")
    print(f"Num_test: {len(base_val)}")

    train_aug_prompts, train_baselines, val_aug_prompts, val_baselines = build_first_step_assignments(base_train, base_val, n_clusters=args.n_clusters, seed=args.seed)
