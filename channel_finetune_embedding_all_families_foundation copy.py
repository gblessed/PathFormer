...
def build_channel_targets(dataset, channels, train, seed=42, train_ratio=0.8):
    ...
    for tx in range(len(dataset_list)):
        ...
        channel_tx = torch.tensor(np.array(channels[:,tx]))
        n_ue = data_tx.n_ue
        ...
        if indices:
            print(f"!! {channel_tx.shape} {channel_tx[indices].shape}")
...
def channel_loss_for_batch(model, batch, model_family, scale=1e6):
    ...
    mse = ((gt_norm.real - pred_norm.real) ** 2 + (gt_norm.imag - pred_norm.imag) ** 2).mean()
    gt_norm_sq = (gt_norm.real ** 2 + gt_norm.imag ** 2).mean().clamp(min=1e-8)
    return mse / (gt_norm_sq + 1e-10)


def channel_loss_from_cached_batch(model, batch, target_scale, scale=1e6):
...
def precompute_cached_channel_features(model, loader, model_family, desc):
    model.eval()
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(train_loader, desc=f"{scenario} [{model_family}] epoch {epoch} train", leave=False):
            if target_scale is None:
                ...
                loss = channel_loss_from_cached_batch(model, batch, target_scale)
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc=f"{scenario} [{model_family}] epoch {epoch} val", leave=False):
                if target_scale is None:
                    ...
                    val_losses.append(channel_loss_from_cached_batch(model, batch, target_scale).item())
...
def run_scenario_family(dataset, channel_targets, scenario, model_family, path_checkpoint, args):
    ...
    if args.use_cached_features:
        cached_train = precompute_cached_channel_features(
            ...
            desc=f"Cache train features [{scenario}]",
        )
        ...
        cached_val = precompute_cached_channel_features(
            ...
            val_loader,
            "first_step_residual_corridor",
            desc=f"Cache val features [{scenario}]",
        )
