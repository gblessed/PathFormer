#!/bin/bash
#SBATCH --job-name=wifo-local-par
...
SCRIPT_PATH="${SCRIPT_DIR}/finetune_user_localization.py"
LOG_DIR="${PROJECT_DIR}/logs"

CHECKPOINT="${CHECKPOINT:-${SCRIPT_DIR}/weights/wifo_base}"
DATA_DIR="${DATA_DIR:-${PROJECT_DIR}/WiFo/dataset/blessed_task_user_loc}"
OUTPUT_ROOT="${OUTPUT_ROOT:-${PROJECT_DIR}/WiFo/localization_finetune_runs_parallel}"
RESULTS_ROOT="${RESULTS_ROOT:-${PROJECT_DIR}/WiFo}"
PER_SCENARIO_DIR="${PER_SCENARIO_DIR:-${RESULTS_ROOT}/localization_finetune_per_scenario_parallel}"
MERGED_CSV="${MERGED_CSV:-${RESULTS_ROOT}/localization_finetune_results_parallel_merged.csv}"
...
echo "Finished WiFo Localization finetuning runs."
