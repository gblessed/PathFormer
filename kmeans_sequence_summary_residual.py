...
def _compute_cluster_timestep_means(padded_paths, path_padding_mask, labels, first_step_centers):
    n_clusters = first_step_centers.shape[0]
    ...
    for cluster_id in range(n_clusters):
        ...
        timestep_means[cluster_id, 1, :] = first_step_centers[cluster_id]
        for t in range(1, max_len):
            ...
            if np.any(valid_t):
                ...
                timestep_means[cluster_id, t, :] = first_step_centers[cluster_id]
...
def _build_sequence_summary_features(paths_list, path_lengths):
    features = []
    for paths, path_length in zip(paths_list, path_lengths):
        ...
        seq = paths[1:1 + n_valid, :2].astype(np.float32)
        delay = seq[:, 0]
        power = seq[:, 1]

        top_k = 3
        padded = np.zeros((top_k, 2), dtype=np.float32)
        take = min(top_k, n_valid)
        padded[:take] = seq[:take]

        total_power = float(np.sum(power))
        mean_delay = float(np.mean(delay))
        std_delay = float(np.std(delay))
...
def _build_train_examples(train_group, n_clusters, random_state):
    summary_features = _build_sequence_summary_features(train_group["paths"], train_group["path_length"])
    feature_mean, feature_std = _fit_standardizer(summary_features)
    summary_features_norm = _apply_standardizer(summary_features, feature_mean, feature_std)

    n_train = summary_features_norm.shape[0]
    ...
    labels = kmeans.fit_predict(summary_features_norm)
    centers_norm = kmeans.cluster_centers_.astype(np.float32)
    stds_norm = _compute_cluster_stats(summary_features_norm, labels, centers_norm)

    center_features = centers_norm[labels]
    std_features = stds_norm[labels]
    ...
    first_step_centers = np.zeros((k_eff, 2), dtype=np.float32)
    for cluster_id in range(k_eff):
        cluster_mask = labels == cluster_id
        if np.any(cluster_mask):
            first_step_centers[cluster_id] = train_group["first_target"][cluster_mask].mean(axis=0).astype(np.float32)
    ...
    residual_paths = _apply_cluster_residual_transform(
        padded_paths,
    ...
    return {
        ...
        "kmeans_centers": centers_norm,
        "kmeans_stds": stds_norm,
        "cluster_timestep_means": cluster_timestep_means,
        "train_rx_pos": train_group["rx_pos"],
        "summary_features_mean": feature_mean,
        "summary_features_std": feature_std,
...
def _build_val_examples(train_kmeans_data, val_group):
    ...
    val_summary_features = _build_sequence_summary_features(val_group["paths"], val_group["path_length"])
    val_summary_features_norm = _apply_standardizer(
        val_summary_features,
        train_kmeans_data["summary_features_mean"],
        train_kmeans_data["summary_features_std"],
    )

    centers = train_kmeans_data["kmeans_centers"]
    dists = np.sum(
        (val_summary_features_norm[:, None, :] - centers[None, :, :]) ** 2,
        axis=2,
    )
    assigned_cluster_ids = np.argmin(dists, axis=1)
...
def _train_model(
    ...
    model.train()
    pbar = tqdm(range(epochs), desc="Summary residual train", leave=False)
    for epoch in pbar:
        epoch_loss = 0.0
        n_batches = 0
...
def evaluate_scenario(
    ...
    val_cluster_first_step_mean_list = []
    val_path_list = []
    ...
    for tx_key, train_group in train_by_tx.items():
        ...
        val_cluster_first_step_mean_list.append(val_examples["timestep_means"][:, 1, :])
    ...
    val_cluster_first_step_means = np.concatenate(val_cluster_first_step_mean_list, axis=0)
    ...
    base_delay_rmse, base_power_rmse, base_joint_rmse = _compute_first_step_metrics(
        val_cluster_first_step_means,
        val_first_targets,
    )
    ...
    return {
        ...
        "summary_kmeans_delay_rmse": base_delay_rmse,
        "summary_kmeans_power_rmse": base_power_rmse,
        "summary_kmeans_joint_rmse": base_joint_rmse,
        ...
        "delay_rmse_gain_vs_summary_kmeans": base_delay_rmse - residual_delay_rmse,
        "power_rmse_gain_vs_summary_kmeans": base_power_rmse - residual_power_rmse,
        "joint_rmse_gain_vs_summary_kmeans": base_joint_rmse - residual_joint_rmse,
    }


def parse_args():
    parser = argparse.ArgumentParser(
        description="Residual PathDecoder conditioned on a KMeans codebook built from compact sequence-summary features."
    )
    parser.add_argument(
        "--scenarios",
        type=str,
    ...
    parser.add_argument(
        "--output-csv",
        type=str,
        default="kmeans_sequence_summary_residual_results.csv",
    )
...
def main():
    ...
    for scenario in scenarios:
        ...
        print(
            f"{scenario} | "
            f"summary_kmeans_delay_rmse={row['summary_kmeans_delay_rmse']:.4f}, "
            f"residual_delay_rmse={row['residual_delay_rmse']:.4f}, "
            f"summary_kmeans_power_rmse={row['summary_kmeans_power_rmse']:.4f}, "
