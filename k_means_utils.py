...
def compute_delay_kmeans_cluster_centers(dataloader_obj, max_path_len=26, n_clusters=4, random_state=42):
    """
    Compute K-means cluster centers for delays at each path step, per Tx.
    Each Tx has its own set of clusters based on its users' delay distributions.
    
    Args:
        dataloader_obj: PreTrainMySeqDataLoader (train) with dataset_filtered
        max_path_len: max path steps to compute clusters for
        n_clusters: number of clusters per step
        random_state: for reproducibility
    
    Returns:
        cluster_centers: np.array of shape (n_tx, max_path_len, n_clusters)
        cluster_centers[tx, t, k] = k-th center at path step t for Tx tx
    """
    df = dataloader_obj.dataset_filtered
    n_samples = len(df["delay"])
    if n_samples == 0:
        return np.zeros((1, max_path_len, n_clusters), dtype=np.float32)
    ...
    # delays_per_tx_per_step[tx][t] = list of delays for Tx tx at step t
    delays_per_tx_per_step = [[[] for _ in range(max_path_len)] for _ in range(n_tx)]
    ...
    for idx in range(n_samples):
        ...
        if dataloader_obj.sort_by == "power":
            indices = np.argsort(-powers_raw)
        else:
            indices = np.argsort(delays_raw)
        ...
        for indx in indices:
            ...
            if np.isnan(d):
                break
            value = d * 1e6  # s -> μs
            if getattr(dataloader_obj, 'normalizers', None) and getattr(dataloader_obj, 'apply_normalizers', None) and "delay" in dataloader_obj.apply_normalizers:
                value = (value - dataloader_obj.normalizers["delay"]["mean"]) / dataloader_obj.normalizers["delay"]["std"]
            delays_per_tx_per_step[tx][step].append(value)
            step += 1
    
    cluster_centers = np.zeros((n_tx, max_path_len, n_clusters), dtype=np.float32)
    
    for tx in range(n_tx):
        for t in range(max_path_len):
            arr = np.array(delays_per_tx_per_step[tx][t], dtype=np.float32)
            if len(arr) < n_clusters:
                if len(arr) > 0:
                    q = np.linspace(0, 100, n_clusters)
                    cluster_centers[tx, t] = np.percentile(arr, q)
                else:
                    cluster_centers[tx, t] = np.zeros(n_clusters)
            else:
                kmeans = KMeans(n_clusters=n_clusters, random_state=random_state, n_init=10)
                arr_2d = arr.reshape(-1, 1)
                kmeans.fit(arr_2d)
                centers = np.sort(kmeans.cluster_centers_.flatten())
                cluster_centers[tx, t] = centers
    
    return cluster_centers


def precompute_train_cluster_embeddings(dataloader_obj, cluster_centers, cluster_embed_fn, max_path_len=26,
                                        n_clusters=4, hidden_dim=512, device="cuda"):
    """
    Precompute cluster embedding sequences for each training sample (indexed by its delays).
    train_rx_pos uses same coordinate transform as prompt (for consistent NN lookup).
    
    Returns:
        train_rx_pos: (N, 2) - rx x,y in same coord system as prompt [3:5]
        train_tx_idx: (N,) - tx index per sample
        train_cluster_embs: (N, max_path_len, hidden_dim) - precomputed cluster embeddings
        train_valid_len: (N,) - actual path length for padding mask
    """
    df = dataloader_obj.dataset_filtered
    n_samples = len(df["delay"])
    ...
    train_rx_pos = []
    train_tx_idx = []
    all_selected = []  # will become (N, max_path_len)
    valid_lens = []
    ...
    for idx in range(n_samples):
        ...
        if norm and "pos" in apply_norm:
            rx_xy = (rx_xy - norm["rx_pos"]["mean"][:2]) / norm["rx_pos"]["std"][:2]
        else:
        ...
        selected = np.zeros(max_path_len, dtype=np.float32)
        step = 0
        for indx in indices:
            if step >= max_path_len:
                break
            ...
            if getattr(dataloader_obj, 'normalizers', None) and getattr(dataloader_obj, 'apply_normalizers', None) and "delay" in dataloader_obj.apply_normalizers:
                value = (value - dataloader_obj.normalizers["delay"]["mean"]) / dataloader_obj.normalizers["delay"]["std"]
            cc_t = cc[tx, step, :]  # (n_clusters,)
            nearest = np.argmin(np.abs(cc_t - value))
            selected[step] = cc_t[nearest]
            step += 1
    ...
    train_rx_pos = np.array(train_rx_pos, dtype=np.float32)
    train_tx_idx = np.array(train_tx_idx, dtype=np.int64)
    all_selected = np.array(all_selected, dtype=np.float32)  # (N, max_path_len)
    train_valid_len = np.array(valid_lens, dtype=np.int64)
    
    # Embed: (N, max_path_len, 1) -> (N, max_path_len, hidden_dim)
    sel_t = torch.tensor(all_selected, dtype=torch.float32, device=device).unsqueeze(-1)
    with torch.no_grad():
        train_cluster_embs = cluster_embed_fn(sel_t)  # (N, max_path_len, H)
    
    return train_rx_pos, train_tx_idx, train_cluster_embs.cpu().numpy(), train_valid_len, sel_t


def precompute_train_cluster_raw(dataloader_obj, cluster_centers, max_path_len=26, device="cuda"):
    """
    Same as precompute_train_cluster_embeddings but returns only raw cluster values (no embedding).
    Use this when cluster_embed is learned: lookup returns raw (B, T, 1), model embeds in forward.
    Returns:
        train_rx_pos, train_tx_idx, train_step_means (N, max_path_len, 1) numpy, train_valid_len
    """
    df = dataloader_obj.dataset_filtered
    n_samples = len(df["delay"])
    cc = np.asarray(cluster_centers)
    ...
    train_rx_pos = np.array(train_rx_pos, dtype=np.float32)
    train_tx_idx = np.array(train_tx_idx, dtype=np.int64)
    train_step_means = np.array(all_selected, dtype=np.float32)[:, :, np.newaxis]
    train_valid_len = np.array(valid_lens, dtype=np.int64)
    return train_rx_pos, train_tx_idx, train_step_means, train_valid_len


def lookup_cluster_embs_by_position(prompts, tx_idx, train_rx_pos, train_tx_idx, train_cluster_embs,
                                    train_valid_len, device, prompt_rx_slice=(3, 5)):
    """
    Find nearest training neighbor by (x,y) per sample; return their cluster embeddings.
    prompts: (B, 6) - [tx_pos(3), rx_pos(3)]; rx_pos is indices 3:6, use 3:5 for x,y
    tx_idx: (B,)
    train_*: from precompute_train_cluster_embeddings
    Returns: cluster_emb (B, max_T, H), pad_mask (B, max_T) True=ignore
    """
    B = prompts.shape[0]
    rx_query = prompts[:, prompt_rx_slice[0]:prompt_rx_slice[1]].cpu().numpy()  # (B, 2)
    max_T = train_cluster_embs.shape[1]
    H = train_cluster_embs.shape[2]
    if torch.is_tensor(train_cluster_embs):
        train_cluster_embs = train_cluster_embs.cpu().numpy()
    else:
        train_cluster_embs = np.asarray(train_cluster_embs, dtype=np.float32)
    cluster_emb_batch = np.zeros((B, max_T, H), dtype=np.float32)
    pad_mask = np.ones((B, max_T), dtype=bool)
    
    for b in range(B):
        tx_b = int(tx_idx[b].item()) if torch.is_tensor(tx_idx[b]) else int(tx_idx[b])
        pos_b = rx_query[b]
        # Find samples with same tx
        ...
        cluster_emb_batch[b] = train_cluster_embs[nn_idx]
        valid = train_valid_len[nn_idx]
        pad_mask[b, :valid] = False
        pad_mask[b, valid:] = True
    
    return (torch.tensor(cluster_emb_batch, dtype=torch.float32, device=device),
            torch.tensor(pad_mask, dtype=torch.bool, device=device))


def lookup_cluster_raw_by_position(prompts, tx_idx, train_rx_pos, train_tx_idx, train_step_means,
                                   train_valid_len, device, prompt_rx_slice=(3, 5)):
    """
    Same NN lookup by (x,y) but returns raw cluster delay values (B, max_T, 1) for learned embedding.
    train_step_means: (N, max_path_len, 1) from precompute (all_selected.unsqueeze(-1)).
    Returns: cluster_raw (B, max_T, 1), pad_mask (B, max_T) True=ignore
    """
    B = prompts.shape[0]
    rx_query = prompts[:, prompt_rx_slice[0]:prompt_rx_slice[1]].cpu().numpy()
    max_T = train_step_means.shape[1]
    if torch.is_tensor(train_step_means):
        train_step_means = train_step_means.cpu().numpy()
    else:
        train_step_means = np.asarray(train_step_means, dtype=np.float32)
    if train_step_means.ndim == 2:
        train_step_means = train_step_means[:, :, np.newaxis]
    cluster_raw_batch = np.zeros((B, max_T, 1), dtype=np.float32)
    pad_mask = np.ones((B, max_T), dtype=bool)
    for b in range(B):
        tx_b = int(tx_idx[b].item()) if torch.is_tensor(tx_idx[b]) else int(tx_idx[b])
        pos_b = rx_query[b]
        ...
        dist = np.sqrt(np.sum((pos_subset - pos_b) ** 2, axis=1))
        nn_idx = idx_subset[np.argmin(dist)]
        cluster_raw_batch[b] = train_step_means[nn_idx]
        valid = train_valid_len[nn_idx]
        pad_mask[b, :valid] = False
        pad_mask[b, valid:] = True
    return (torch.tensor(cluster_raw_batch, dtype=torch.float32, device=device),
            torch.tensor(pad_mask, dtype=torch.bool, device=device))
...
def generate_paths_no_env(model, prompt, max_steps=25, stop_threshold=0.5, cluster_emb=None, cluster_pad_mask=None):
    """
    Generate paths autoregressively.
    cluster_emb: (1, max_T, H) - precomputed from nearest train neighbor by (x,y). Required when use_delay_clusters.
    cluster_pad_mask: (1, max_T) True = ignore. Pass from lookup_cluster_embs_by_position.
    """
    model.eval()
    device = next(model.parameters()).device
    prompt = prompt.unsqueeze(0).to(device)  # (1, prompt_dim)

    # Start with SOS tokens (delay, power, phase, aoa_az, aoa_el)
    cur = torch.zeros(1, 1, 5, device=device)  # (1, 1, 5)
    inter_str = -1 * torch.ones(1, 1, 4, device=device)  # (1, 1, 4) - interaction labels

    outputs = []
    outputs_inter_str = []

    for t in range(max_steps):
        # Forward pass - cluster_emb from NN lookup by position (same at train and test)
        d, p, s, c, ph, az_s, az_c, az, el_s, el_c, el, pathcounts, inter_str_logits = model(
            prompt, cur, inter_str, cluster_emb=cluster_emb, cluster_pad_mask=cluster_pad_mask
        )
...
def masked_loss(delay_pred, power_pred, phase_sin_pred, phase_cos_pred, phase_pred,
                ...
                delay_only=False, path_padding_mask=None):
    """
    Added interaction prediction loss as auxiliary task.
    ...
    Args:
        ...
        interaction_targets: (B, T, 4) - binary labels, -1 for invalid
        interaction_weight: weight for interaction loss
        path_padding_mask: (B, T_path) True=valid. If given, used instead of pad_value for mask.
    """
    delay_t = targets[:, :, 0]
    power_t = targets[:, :, 1]
    phase_t = targets[:, :, 2]
