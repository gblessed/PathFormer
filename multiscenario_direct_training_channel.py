...
def compute_stop_metrics(path_count, targets, pad_value=0):
    """
    RMSE between predicted path count and targets.
    Handles NaN by replacing with 0 before sklearn (avoids crash; RMSE may be off if many NaNs).
    """
    y_pred = path_count.cpu().numpy().flatten()
    y_true = targets.squeeze().cpu().numpy().flatten()
    # sklearn's mean_squared_error rejects NaN; replace with 0 to avoid crash
...
def evaluate_model(model, val_loader, max_generate=26, log_to_wandb=False):
    ...
    with torch.no_grad():
        ...
        for prompts, paths, path_lengths, interactions, env, env_prop, path_padding_mask in outer_bar:
            ...
            for b in inner_bar:
                ...
                # Channel NMSE (log scale) from autoregressively generated paths
                if T > 0:
                    params = ChannelParameters()
                    # GT channel: (1, T) for batch format
                    delay_secs_gt = gt[:, 0] / 1e6
                    power_linear_gt = 10**((gt[:, 1] / 0.01) / 10)
                    phase_degs_gt = torch.rad2deg(gt[:, 2])
                    array_resp_gt = compute_single_array_response_torch(params.bs_antenna, gt[:, 3].unsqueeze(0), gt[:, 4].unsqueeze(0))
                    dopplers = torch.zeros(1, T, device=gt.device)
                    gt_ch = generate_MIMO_channel_torch(array_resp_gt, power_linear_gt.unsqueeze(0), delay_secs_gt.unsqueeze(0), phase_degs_gt.unsqueeze(0), dopplers, ofdm_params=params.ofdm, freq_domain=params.freq_domain)
                    # Pred channel
                    ...
                    array_resp_pred = compute_single_array_response_torch(params.bs_antenna, pred[:, 3].unsqueeze(0), pred[:, 4].unsqueeze(0))
                    pred_ch = generate_MIMO_channel_torch(array_resp_pred, power_linear_pred.unsqueeze(0), delay_secs_pred.unsqueeze(0), phase_degs_pred.unsqueeze(0), dopplers, ofdm_params=params.ofdm, freq_domain=params.freq_domain)
                    # NMSE and log10(NMSE)
                    scale = 1e6
                    gt_s = gt_ch * scale
                ...
                if ch_nmse_log_list:
                    postfix_dict["log_nmse"] = f"{ch_nmse_log_list[-1]:.3f}"
    ...
    return avg_delay, avg_power, avg_phase, avg_az, avg_el, avg_path_length_rmse, avg_delay_mae, avg_power_mae, avg_phase_mae, avg_az_mae, avg_el_mae, avg_path_length_mae, avg_ch_nmse_log 


def evaluate_model_batch(model, val_loader, max_generate=26, log_to_wandb=False):
    """Batch inference: one forward per batch instead of per sample. Same metrics as evaluate_model."""
        ...
        for prompts, paths, path_lengths, interactions, env, env_prop, path_padding_mask in outer_bar:
            ...
            for b in range(B):
                ...
                if T > 0:
                    ...
                    gt_norm_sq = (gt_s.real**2 + gt_s.imag**2).mean().item()
                    nmse = mse / (max(gt_norm_sq, 1e-6) + 1e-10)
                    ch_nmse_log_list.append(np.log10(nmse + 1e-10))
                delay_errors.append(delay_rmse)
                power_errors.append(power_rmse)
                phase_errors.append(phase_rmse)
                path_length_rmses.append(length_rmse)
...
def train_with_interactions(model, train_loader, val_loader, config, train_data, task=None):
    ...
    for epoch in range(config["epochs"]):
        ...
        for prompts, paths, path_lengths, interactions, env, env_prop, path_padding_mask in pbar:  # NEW: added interactions
            ...
            optimizer.zero_grad()
            total_loss.backward()
            grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), config.get("GRAD_CLIP_NORM", 1.0))
            optimizer.step()
            ...
            ch_nmse_val = loss_channel.item() if torch.is_tensor(loss_channel) else float(loss_channel)
            train_ch_nmse.append(ch_nmse_val)
        ...
        avg_train_ch_nmse = np.mean(train_ch_nmse) if train_ch_nmse else 0.0
        ...
        with torch.no_grad():
            ...
            for prompts, paths, path_lengths, interactions, env, env_prop, path_padding_mask in pbar:  # NEW
                ...
                ch_nmse_val = loss_channel.item() if torch.is_tensor(loss_channel) else float(loss_channel)
                val_ch_nmse.append(ch_nmse_val)

                pbar.set_postfix({
                    "val_loss": f"{total_loss.item():.4f}",
                    "inter": f"{loss_interaction.item():.4f}",  # NEW
                    "ch_nmse": f"{ch_nmse_val:.4f}",
                })
        ...
        avg_val_ch_nmse = np.mean(val_ch_nmse) if val_ch_nmse else 0.0
        ...
        if config.get("USE_WANDB", False):
            ...
            wandb.log({
                ...
                "train_ch_nmse": avg_train_ch_nmse,
        ...
        print(f"    Channel NMSE: {avg_train_ch_nmse:.4f} (val: {avg_val_ch_nmse:.4f})")
...
# %%
    ...
    # Train
    train_with_interactions(model, train_loader, val_loader, config, train_data, config["task"])
    ...
    # print(results)
    avg_delay, avg_power, avg_phase, avg_az, avg_el, avg_path_length_rmse, avg_delay_mae, avg_power_mae, avg_phase_mae, avg_az_mae, avg_el_mae, avg_path_length_mae, avg_ch_nmse_log = results
