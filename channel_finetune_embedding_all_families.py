...
def build_gt_channel_batch(paths_out, path_padding_mask):
    params = ChannelParameters()
    ...
    # Channel generation is BS-array based, so use AoD. The helper expects
    # theta=elevation and phi=azimuth.
    aod_az = paths_out[:, :, 5]
    aod_el = paths_out[:, :, 6]
    ...
    dopplers = torch.zeros(B, T, device=paths_out.device, dtype=paths_out.dtype)
...
def unpack_batch(batch, model_family):
    if model_family == "direct":
        ...
        prompts, paths, path_lengths, interactions, env, env_prop, path_padding_mask, first_step_baseline = batch
    ...
    return prompts, paths, interactions, path_padding_mask, first_step_baseline
...
def run_scenario_family(dataset, scenario, model_family, path_checkpoint, args):
    if not os.path.exists(path_checkpoint):
        print(f"Skipping {scenario} [{model_family}] missing path checkpoint: {path_checkpoint}")
        return None

    pad_value = 0
    _, val_data, train_loader, val_loader, prompt_dim = build_loaders_for_family(dataset, model_family, args, pad_value)
    if model_family == "direct":
        prompt_dim = None
...
def run_scenario(scenario, args):
    ...
    for model_family, path_checkpoint in checkpoint_specs_for_scenario(scenario, args):
        row = run_scenario_family(dataset, scenario, model_family, path_checkpoint, args)
        if row is not None:
            rows.append(row)
